    ...
    while (*fmt) {
        if (*fmt == '%') {
            ...
            switch(*fmt++) {
                ...
                break;
                case 'd':
                    puts(print_lint((long)*(int *)arg_ptr, 10, tmp_buff, TMP_BUFF_SIZE));
                    arg_ptr += sizeof(int);
                break;
                case 'u':
                    puts(print_ulint((unsigned long)(*(unsigned int *)arg_ptr), 10, tmp_buff, TMP_BUFF_SIZE));
                    arg_ptr += sizeof(unsigned int);
                break;
                case 'x':
                    puts(print_ulint((unsigned long)(*(unsigned int *)arg_ptr), 16, tmp_buff, TMP_BUFF_SIZE));
                    arg_ptr += sizeof(unsigned int);
                break;
                case 'b':
                    puts(print_ulint((unsigned long)(*(unsigned int *)arg_ptr), 2, tmp_buff, TMP_BUFF_SIZE));
                    arg_ptr += sizeof(unsigned int);
                break;
                case 'l':
                    switch (*fmt++) {
